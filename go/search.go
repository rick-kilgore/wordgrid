...
func (fw FoundWord) String() string {
	return asString(fw.word, fw.score, fw.dirn, fw.pos, fw.used)
}

func (fw FoundWord) AsResult(grid Grid) string {
	colored, err := grid.ColorWord(fw.word, fw.pos, fw.dirn)
	if err != nil {
		return fw.String()
	}
	return asString(colored, fw.score, fw.dirn, fw.pos, fw.used)
}

func asString(word string, score int, dirn Dir, pos CPos, used int) string {
	return fmt.Sprintf("%d: %s %s from %s (%d ltrs)", score, word, dirn, pos, used)
}
